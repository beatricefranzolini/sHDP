...
for mm in selected:
    ...
    for i in range(100):
        for ii in range(i+1,100):
            ccp[i,ii] = np.count_nonzero(xi_final[5000:10000,i]==xi_final[5000:10000,ii])
            ccp[ii,i] = ccp[i,ii]
    ...
    fig =P1.get_figure()        
    fig.savefig('cluster_scenario0%s' % mm, dpi=300)
    #overall order
    for i in range(100):
        ccp[i,i]=1
    ...
    for i in range(5000,10000):
        K[i-5001]=len(np.unique(xi_final[i,:]))
    unique, counts = np.unique(K, return_counts=True)    
    plt.bar(unique, counts/5000, width = 0.5, color=cmap(1000))
    plt.savefig('K_secondlevel_scenario0_shdp%s' %mm, dpi=300)